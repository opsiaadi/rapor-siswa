        ...
        <table class="w-full text-sm text-left text-gray-500 ">
            ...
            @forelse($data as $index => $siswa)
                <tr class="bg-white border-b border-gray-100 hover:bg-gray-50 transition-colors group">
                    ...
                    <td class="px-6 py-4">
                        <span class="px-3 py-1 bg-blue-50 text-blue-700 rounded-full text-xs font-bold uppercase tracking-tighter">{{ $siswa->jenis_kelamin?->value ?? '-' }}</span>
                    </td>
        ...
        <div class="p-4 hover:bg-gray-50 transition-colors">
            <div class="flex items-start justify-between mb-3">
                ...
                <div class="flex items-center gap-1">
                    <button type="button"
                            data-modal-target="detailModal"
                            data-modal-toggle="detailModal"
                            onclick="openDetailModal('{{ strtoupper(substr($siswa->nama ?? 'S', 0, 1)) }}', '{{ $siswa->nama ?? '-' }}', '{{ $siswa->nis ?? '-' }}', '{{ $siswa->jenis_kelamin?->value ?? '-' }}', '{{ $siswa->tahun_ajaran ?? '-' }}', '{{ $siswa->kelas->nama_kelas ?? '-' }}', '{{ $siswa->kelas->waliKelas->nama ?? '-' }}')" class="text-gray-500 hover:text-blue-600 p-2 rounded-lg hover:bg-blue-50 transition-all" title="Detail Siswa">
            ...
            <div class="grid grid-cols-2 gap-2 text-xs">
                ...
                <div>
                    ...
                    <p><span class="px-2 py-0.5 bg-blue-50 text-blue-700 rounded-full text-xs font-bold">{{ $siswa->jenis_kelamin?->value ?? '-' }}</span></p>
                </div>
                <div>
