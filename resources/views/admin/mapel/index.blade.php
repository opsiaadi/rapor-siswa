...
<div class="space-y-6">
    ...
    <div class="bg-white  rounded-xl border border-gray-200  shadow-sm p-5 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div class="flex items-center gap-4">
            ...
            <div>
                ...
                <p class="text-sm text-gray-500 ">Total <span class="text-amber-600  font-bold">{{ count($data ?? []) }}</span> Mata Pelajaran</p>
            </div>
        </div>
    ...
    <div class="bg-white  rounded-xl border border-gray-200  shadow-sm overflow-hidden">
            ...
            <table class="w-full text-sm text-left text-gray-500 ">
                ...
                <tbody class="divide-y divide-gray-200 ">
                    ...
                    <tr class="bg-white  hover:bg-gray-50  transition-colors">
                        ...
                        <td class="px-6 py-4 text-gray-700 ">{{ $m->kode_mapel ?: '-' }}</td>
                        <td class="px-6 py-4 font-semibold text-gray-900 ">{{ $m->nama }}</td>
                        <td class="px-6 py-4">
                            <span class="px-3 py-1 rounded-full text-xs font-bold {{ $m->kkm >= 75 ? 'bg-green-50 text-green-700' : 'bg-yellow-50 text-yellow-700' }}">
                                {{ $m->kkm }}
                            </span>
                        </td>
                        <td class="px-6 py-4 text-center">
                            <div class="flex items-center justify-center gap-2">
                                <a href="{{ route('admin.mapel.edit', $m->id) }}" class="text-blue-600 hover:text-blue-800   transition-colors">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/>
                                    </svg>
                                </a>
                                <form action="{{ route('admin.mapel.destroy', $m->id) }}" method="POST" class="inline" onsubmit="return confirm('Hapus data ini?')">
                                    @csrf
                                    @method('DELETE')
                    ...
                    <tr class="bg-white ">
                        <td colspan="5" class="px-6 py-16 text-center">
                            <div class="flex flex-col items-center gap-3">
                                ...
                                <div>
                                    <p class="text-sm font-semibold text-gray-600 ">Belum ada data mata pelajaran</p>
                                    <p class="text-xs text-gray-400  mt-1">Klik "Tambah Mapel" untuk menambahkan</p>
                                </div>
                            </div>
                        </td>
                    </tr>
        ...
        <div class="px-6 py-4 bg-gray-50  border-t border-gray-200  flex items-center justify-between">
            <p class="text-xs text-gray-500  font-medium">Showing <span class="font-bold text-gray-900 ">1-{{ count($data ?? []) }}</span> of <span class="font-bold text-gray-900 ">{{ count($data ?? []) }}</span> subjects</p>
