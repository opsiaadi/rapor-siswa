    ...
    <div class="block md:hidden divide-y divide-gray-100">
        ...
        <div class="p-4 hover:bg-gray-50 transition-colors">
            <div class="flex items-start justify-between mb-3">
                <div>
                    <p class="font-semibold text-gray-900 text-sm">{{ $mapel->nama_mapel }}</p>
                    <p class="text-xs text-gray-500">#{{ str_pad($index + 1, 2, '0', STR_PAD_LEFT) }}</p>
                </div>
                <div class="flex items-center gap-1">
                    <a href="{{ route('admin.mapel.edit', $mapel->id) }}" class="text-gray-500 hover:text-amber-600 p-2 rounded-lg hover:bg-amber-50 transition-all" title="Edit">
            ...
            <div class="grid grid-cols-2 gap-2 text-xs">
                <div>
                    ...
                    <p class="font-medium text-gray-900 font-mono">{{ $mapel->kode_mapel ?? 'MP-'.str_pad($mapel->id, 3, '0', STR_PAD_LEFT) }}</p>
                </div>
                <div>
                    <span class="text-gray-400">KKM</span>
                    <p><span class="px-2.5 py-0.5 bg-green-50 text-green-700 rounded-full text-xs font-bold uppercase">{{ $mapel->kkm }}</span></p>
                </div>
            </div>
        </div>
        @empty
