@extends('layouts.admin', [
    'title' => 'Edit Eskul',
    'pageTitle' => 'Edit Eskul',
    'breadcrumb' => 'Ekstrakurikuler › Edit Eskul',
...
<div class="space-y-6">
    ...
    <div class="p-4 bg-white rounded-lg border border-gray-200 shadow-sm">
        <div class="flex items-center gap-4">
            ...
            <div>
                <h2 class="text-lg font-semibold text-gray-900">Edit Eskul</h2>
                <p class="text-sm text-gray-500 mt-1">Ubah data ekstrakurikuler</p>
            </div>
        </div>
    </div>

    <!-- Form -->
    <form action="{{ route('admin.eskul.update', $eskul->id) }}" method="POST">
        @csrf
        @method('PUT')
                ...
                <div>
                    ...
                    <div class="flex gap-6">
                        <label class="flex items-center gap-2 cursor-pointer">
                            <input type="radio" name="is_aktif" value="1" {{ old('is_aktif', $eskul->is_aktif ? '1' : '0') == '1' ? 'checked' : '' }}
                                class="w-4 h-4 text-orange-600 border-gray-300 focus:ring-orange-500">
                            <span class="text-sm text-gray-700">Aktif</span>
                        </label>
                        <label class="flex items-center gap-2 cursor-pointer">
                            <input type="radio" name="is_aktif" value="0" {{ old('is_aktif', $eskul->is_aktif ? '1' : '0') == '0' ? 'checked' : '' }}
                                class="w-4 h-4 text-orange-600 border-gray-300 focus:ring-orange-500">
