...
<div class="bg-white rounded-xl border border-gray-200 shadow-sm p-5 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
    ...
    <div class="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 w-full md:w-auto">
        ...
        <a href="{{ route('admin.guru.create') }}" class="text-white bg-purple-600 hover:bg-purple-700 focus:ring-4 focus:ring-purple-300 font-semibold rounded-full text-sm px-5 py-2.5 flex items-center justify-center gap-2 transition-all duration-200 hover:scale-105 hover:shadow-lg active:scale-95">
                    ...
                    <tr class="bg-white border-b border-gray-100 hover:bg-gray-50 transition-colors group">
                        ...
                        <td class="px-6 py-4">
                            <div class="flex items-center justify-center gap-1">
                                <a href="{{ route('admin.guru.edit', $guru->id) }}" class="text-gray-500 hover:text-amber-600 p-1.5 rounded-lg hover:bg-amber-50 transition-all duration-200 hover:scale-110 active:scale-95" title="Edit">
    ...
    <div class="block md:hidden divide-y divide-gray-100">
        ...
        <div class="p-4 hover:bg-gray-50 transition-colors">
            <div class="flex items-start justify-between mb-3">
                <div class="flex items-center gap-3">
                    <div class="w-10 h-10 rounded-full bg-purple-100 text-purple-600 flex items-center justify-center text-sm font-bold">
                        {{ strtoupper(substr($guru->nama ?? 'G', 0, 1)) }}
                    </div>
                    <div>
                        <p class="font-semibold text-gray-900 text-sm">{{ $guru->nama ?? '-' }}</p>
                        <p class="text-xs text-gray-500">#{{ str_pad($index + 1, 2, '0', STR_PAD_LEFT) }}</p>
                    </div>
                </div>
                <div class="flex items-center gap-1">
                    <a href="{{ route('admin.guru.edit', $guru->id) }}" class="text-gray-500 hover:text-amber-600 p-2 rounded-lg hover:bg-amber-50 transition-all" title="Edit">
            ...
            <div class="grid grid-cols-2 gap-2 text-xs">
                <div>
                    ...
                    <p class="font-medium text-gray-900 font-mono">{{ $guru->nik ?? '-' }}</p>
                </div>
                <div>
